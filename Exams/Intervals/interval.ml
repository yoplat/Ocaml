...
module Interval (Endpoint : Comparable) :
  ...
  let create low high =
    if Endpoint.compare low high > 0 then raise WrongInterval else if Endpoint.compare low high == 0 then Empty
    else Interval (low,high)
    (** Returns true iff the interval is empty *) let is_empty = function
      | Empty -> true
      | Interval _ -> false
  ...
  let intersect t1 t2 =
    let min x y = if Endpoint.compare x y <= 0 then x else y in let max x y = if Endpoint.compare x y >= 0 then x else y in match t1,t2 with
